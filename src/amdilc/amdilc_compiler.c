    ...
    uint32_t ilElementType)
{
    ...
    case IL_ELEMENTFORMAT_UINT:
        return compiler->uintId;
    case IL_ELEMENTFORMAT_SINT:
        return compiler->intId;
    case IL_ELEMENTFORMAT_UNKNOWN:
    case IL_ELEMENTFORMAT_FLOAT:
    case IL_ELEMENTFORMAT_SNORM:
    case IL_ELEMENTFORMAT_UNORM:
        return compiler->floatId;
    default:
        return 0;
    }
}
    ...
    uint32_t ilElementType)
{
    switch (ilElementType) {
    case IL_ELEMENTFORMAT_UINT:
        return compiler->uint4Id;
    case IL_ELEMENTFORMAT_SINT:
        return compiler->int4Id;
    case IL_ELEMENTFORMAT_UNKNOWN:
    case IL_ELEMENTFORMAT_FLOAT:
    case IL_ELEMENTFORMAT_SNORM:
    case IL_ELEMENTFORMAT_UNORM:
        return compiler->float4Id;
    default:
        return 0;
    }
}
    ...
    const Instruction* instr)
{
    ...
    case IL_OP_AND:
        ...
        break;
    case IL_OP_I_MAD: {
        IlcSpvId mulId = ilcSpvPutAlu(compiler->module, SpvOpIMul, compiler->int4Id,
                                      2, srcIds);
...
{
    IlcSpvId valueId = loadSource(compiler, &instr->srcs[0], COMP_MASK_X, compiler->intId);
    ...
    const Instruction* instr)
{
    ...
    case IL_OP_FETCH4_C: {
        depthComparison = true;
        drefOrComponentId = loadSource(compiler, &instr->srcs[1], mask, compiler->floatId);
    } break;
    ...
    if (drefOrComponentId == 0) {
        ...
        drefOrComponentId = ilcSpvPutConstant(compiler->module, compiler->uintId, componentValue);
    }
    if (GET_BIT(instr->control, 13)) {
        if (offsetPresent) {
                                           ...
                                           IlcSpvId addressId, IlcSpvId strideId, IlcSpvId resultAddressTypeId)
{
    ...
    if (strideId != 0) {
        ...
        const IlcSpvId divIds[] = {
            indexId, ilcSpvPutConstant(compiler->module, compiler->intId, 4)
        };
        return ilcSpvPutAlu(compiler->module, SpvOpSDiv, resultAddressTypeId, 2, divIds);
    }
}
                               ...
                               bool isUav, const Destination* dst)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        IlcSpvId addIds[2] = {addressId, ilcSpvPutConstant(compiler->module, compiler->intId, i)};
    ...
    const Instruction* instr)
{
    ...
    IlcSpvId fetchId = emitBufferLoad(compiler,
                                      ...
                                      addrId, compiler->intId,
                                      false, dst);
    if (compiler->float4Id != sampledTypeId) {
        fetchId = ilcSpvPutBitcast(compiler->module, compiler->float4Id, fetchId);
    }
    storeDestination(compiler, dst, fetchId);
}
