    ...
    const Instruction* instr)
{
    ...
    IlcSpvId srcId = loadSource(compiler, &instr->srcs[0], COMP_MASK_X, compiler->intId);
    IlcSpvId resourceId = ilcSpvPutLoad(compiler->module, resource->typeId, resource->id);
    IlcSpvId fetchId = ilcSpvPutImageFetch(compiler->module, compiler->uint4Id, resourceId, srcId);
    ...
    const Instruction* instr)
{
    ...
    IlcSpvId resourceId = ilcSpvPutLoad(compiler->module, resource->typeId, resource->id);
    IlcSpvId fetchId = ilcSpvPutImageFetch(compiler->module, compiler->uint4Id, resourceId,
                                           wordAddrId);
    storeDestination(compiler, dst,
                     ilcSpvPutBitcast(compiler->module, compiler->float4Id, fetchId));
}
