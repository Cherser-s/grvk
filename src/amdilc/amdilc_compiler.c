...
#define COMP_MASK_XYZ       (COMP_MASK_X | COMP_MASK_Y | COMP_MASK_Z)
...
static IlcResource* addResource(
    IlcCompiler* compiler,
    ...
    IlcCompiler* compiler, IlcSpvId scalarTypeId, uint32_t additionalElementCount)
{
    ...
    consistuentIds[additionalElementCount + 1] = ilcSpvPutConstant(compiler->module, compiler->floatId, ONE_LITERAL);
    ...
    IlcSpvId typeId)
{
    ...
    if (sourceComponents > 1 && targetComponents > 1 &&
        ...
        IlcSpvId zeroOneId = emitZeroOneVector(compiler, sourceScalarTypeId, 4 - sourceComponents);
    ...
    const Instruction* instr)
{
    ...
    IlcSpvId imageId = ilcSpvPutImageType(compiler->module, compiler->float4Id, dim,
    ...
    uint8_t ilSamplerId)
{
    ...
    compiler->samplerResources[ilSamplerId] = ilcSpvPutVariable(compiler->module, compiler->samplerId,
    ...
    const Instruction* instr)
{
    ...
    IlcSpvId samplerId = emitOrGetSampler(compiler, ilSamplerId);
    ...
    case IL_OP_SAMPLE_C_L: {
        depthComparison = true;
        drefId = loadSource(compiler, &instr->srcs[1], mask, compiler->floatId);
        parameters[argCount] = ilcSpvPutConstant(compiler->module, compiler->floatId, ZERO_LITERAL);// lod is zero here
        argCount++;
        argMask |= SpvImageOperandsLodMask;
    } break;
    ...
    if (GET_BIT(instr->control, 13)) {
        IlcSpvId offsetValues[3];//TODO: add support for 3d images
        ...
        IlcSpvId coordIntTypeId = ilcSpvPutVectorType(compiler->module, compiler->intId, 2);
        parameters[argCount] = ilcSpvPutConstantComposite(compiler->module, coordIntTypeId, 2, offsetValues);
        argCount++;
    }
    ...
    const Instruction* instr)
{
    ...
    case IL_OP_ELSE:
        ...
        break;
    case IL_OP_END:
        LOGT("End stream instruction hit\n");
        ilcSpvPutFunctionEnd(compiler->module);
        break;
    case IL_OP_ENDMAIN:
        LOGT("End main instruction hit\n");
        break;
    case IL_OP_ENDIF:
